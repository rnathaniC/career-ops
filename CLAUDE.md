...
- **AUTO-FILL TO SUBMIT READY — Fresh lane (supersedes the 2026-06-24 auto-submit override; re-scoped 2026-08-02, authorized by Rahil, Product Owner).** The Fresh lane no longer attempts an automated submit CLICK. Automated clicks on Greenhouse/Lever/Ashby submit buttons are silently blocked by reCAPTCHA bot-detection (the confirmed 0% ceiling: a human click passes the gate, an automated one does not). So Fresh-lane cards that pass the per-card gates (grade A/B/C eligibility + readiness band >=60 + 5/day cap) are auto-filled and moved to the **Submit Ready** lane, where Rahil makes the final human Submit click, after which the card moves to **Applied**. The readiness `<60 = skip` band remains the quality floor and is NOT bypassed. Nightly staging runs via `npm run auto-submit:park` (Step 5 of `pulse-refresh.mjs`); the interactive fill+submit Rahil drives uses `--semi-auto` or the Claude-in-Chrome flow. The old `--live` auto-click mode still exists for manual/experimental use but is no longer wired into the nightly pipeline.

...
function checkNoNullBytes() {
  // K-2026-06-21 — recursive NUL-byte sweep over all text files. Restored + widened
  // after v1.12.0 dropped the check: a git-checkout-through-mount corrupted 14 files
  // across .github/ modes/ docs/ dashboard/ templates/ .claude/ and the old
  // '.'+'scripts' .mjs/.js/.json scan would have caught only 1.
  // Keep EXTS in sync with scripts/fix-nul-bytes.mjs.
  ...
  const walk = (dir, rel) => {
    ...
    for (const ent of entries) {
      ...
      if (ent.isDirectory()) { walk(fp, r); continue; }
      if (!ent.isFile()) continue;
      if (!exts.some((e) => name.endsWith(e)) || isBackup(name)) continue;
      try { if (readFileSync(fp).includes(0)) offenders.push(r); } catch {}
    }
  };
  walk(projectRoot, '');
  if (offenders.length === 0) return { pass: true, label: 'No NUL-byte corruption in text files' };
  return {
    pass: false,
